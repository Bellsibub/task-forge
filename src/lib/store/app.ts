        ...
        immer((set, get) => ({
            ...
            updateTaskStatus: (taskId, columnId, boardId) =>
                ...
                set((state) => {
                    ...
                    if (state.activeTask?.id === taskId && state.activeTask.subtasks) {
                        const activeSubTaskIndex = state.activeTask.subtasks.findIndex(
                            (s) => s.id === subTaskId,
                        );
                        if (activeSubTaskIndex !== -1) {
                            state.activeTask.subtasks[activeSubTaskIndex].isCompleted = isCompleted;
                        }
                    }
                }),
        })),
